...
HoldNeutral::HoldNeutral(std::shared_ptr<MotionController> motion) : BehaviorNode("HoldNeutral"), _motion(std::move(motion)), _lastTarget(0,0) {}

BT::Status HoldNeutral::tick(const WorldState& ws) {
    if (ws.lastBallLostTime > 500) {
